...
async function main() {
  ...
  console.log(`Starting AI PM Batch Simulation for ${cases.length} cases...\n`);
  ...
  fs.writeFileSync(
    path.join(outputDir, "ai_pm_simulation_results.json"), 
  ...
  fs.writeFileSync(path.join(outputDir, "ai_pm_simulation_summary.csv"), csvHeaders + csvRows);

  console.log("\n✅ AI PM Batch Simulation Complete!");
  console.log(`Saved output to /output/ai_pm_simulation_results.json and /output/ai_pm_simulation_summary.csv`);
}
